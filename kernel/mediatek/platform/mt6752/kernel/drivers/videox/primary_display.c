...
//Lenovo-sw wuwl10 add 20140826 for lcm effect begin
	...
	if(pgc->plcm->drv)
	{
		...
		if(pgc->plcm->drv->set_inversemode)
			disp_feature_info[MTKFB_DISPIF_PRIMARY_LCD].inverse_support = 0;
...
int primary_display_get_lanenum(void)
{
	if(pgc->plcm == NULL)
	{
		DISPERR("lcm handle is null\n");
	...
	if(pgc->plcm->params)
	{
		return pgc->plcm->params->dsi.LANE_NUM;
	}
	else
	{
		...
		{	//cmdq
			...
			if(primary_display_is_video_mode())
			{
				...
				if (!pgc->plcm->drv->set_cabcmode) {
					...
					DISPCHECK("[Display]set_cabcmode NULL\n");
					goto End;
				}
...
DISP_STATUS DISP_SetInverseMode(unsigned int mode)
{
	...
	{
		DISPCHECK("Sleep State set cmd invald\n");
		printk("wuwl Sleep State set cmd invald\n");
	}
	else
	{
		if(primary_display_cmdq_enabled())	
		{	//cmdq
			...
			if(primary_display_is_video_mode())
			{
				...
				if (!pgc->plcm->drv->set_inversemode) {
					ret = DISP_STATUS_NOT_IMPLEMENTED;
					DISPCHECK("[Display]set_inversemode NULL\n");
					goto End;
				}
	...
	else
	{
		...
		{	//cmdq
			...
			if(primary_display_is_video_mode())
			{
				...
				if (!pgc->plcm->drv->set_iemode) {
					ret = DISP_STATUS_NOT_IMPLEMENTED;
					DISPCHECK("[Display]set_iemode NULL\n");
					goto End;
				}
