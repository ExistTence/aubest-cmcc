...
/*lenovo-sw wuwl10 20140815 add "proc/lcm_info" to get lcm info */
...
//Lenovo-sw wuwl10 add 20140826 for lcm effect begin
...
#endif
//Lenovo-sw wuwl10 add 20140826 for lcm effect end 
...
/*lenovo-sw wuwl10 20140815 : add "proc/lcm_info" to show lcm info begin*/
#ifdef LENOVO_LCM_INFO_SHOW
static struct proc_dir_entry *lcm_info_proc=NULL;

static int lcm_info_read_proc(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
	char *page = NULL;
	char *ptr = NULL;
	int len, err = -1;
	...
	ptr = page;
	ptr += sprintf(ptr, "name=%s; width=%d; height=%d; mode=%s; lane=%d;\n",
		mtkfb_lcm_name,
		primary_display_get_width(),
		primary_display_get_height(),
		(primary_display_is_video_mode()==0)?"CMD":"VDO",
		primary_display_get_lanenum()
		);
...
static const struct file_operations lcm_info_proc_fops = {
	.read = lcm_info_read_proc
};
#endif
/*lenovo-sw wuwl10 20140815 : add "proc/lcm_info" to show lcm info end*/
...
#ifdef LENOVO_LCM_INFO_SHOW
	do{
		lcm_info_proc = proc_create("lcm_info",0444,NULL,&lcm_info_proc_fops);
		if(lcm_info_proc==NULL){
			printk("[ywuwl10] Creat lcm_info failed!!!\n");
		}
		else{
			printk("[wuwl10] Create lcm_info entry success!\n");
		}
	}while(0);
#endif
/*lenovo-sw wuwl10 20140815 : add "proc/lcm_info" to show lcm info end*/
