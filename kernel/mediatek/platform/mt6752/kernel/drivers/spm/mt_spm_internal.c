...
wake_reason_t __spm_output_wake_reason(const struct wake_status *wakesta,
				       ...
				       bool suspend)
{
    /* lenovo.sw begin chenyb1 20140818 add for pm log */
#ifdef LENOVO_PM_LOG
       extern void pmlog_save_wake_reason(char *reason_buf, int len);

#endif
    /* lenovo.sw end chenyb1 20140818 add for pm log */
	int i;
    ...
    /* lenovo.sw begin chenyb1 20140818 add for pm log */
#ifdef LENOVO_PM_LOG
       pmlog_save_wake_reason(buf, strlen(buf));
#endif
    /* lenovo.sw end chenyb1 20140818 add for pm log */
