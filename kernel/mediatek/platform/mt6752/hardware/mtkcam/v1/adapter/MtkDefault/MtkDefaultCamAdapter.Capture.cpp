...
onCaptureThreadLoop()
{
    ...
    else
    {
	    ...
	    //songwei add for front camera mirror operation.
	    shotParam.ms8SnapShotFilp           = pParamsMgr->getStr("snapshot-picture-flip");
	    MY_LOGE("----------[%d-%d-%d-%s]\n",shotParam.mu4Transform,shotParam.mi4PostviewWidth,
	                                        shotParam.mi4PostviewHeight,shotParam.ms8SnapShotFilp.string());
	    //songwei add for front camera mirror operation end.
            ...
            if(!mbTakePicPrvNotStop)
            {
                mpDefaultBufHdl->mapPort(DefaultBufHandler::eBuf_Disp, PASS2_PRV_DST_0, MTRUE);
                mpDefaultBufHdl->mapPort(DefaultBufHandler::eBuf_Disp, PASS2_CAP_DST_0);
            }
            //
            ret =
