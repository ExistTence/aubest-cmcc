...
MINT32 EisHalImp::Init()
{
    ...
    mpSensorListener->setListener(mySensorListener);
    mpSensorListener->enableSensor(SensorListener::SensorType_Acc,33);
    mpSensorListener->enableSensor(SensorListener::SensorType_Gyro,33);
    ...
    if(aEisPass == EIS_PASS_1)
    {
        ...
        if(mIsEisConfig == 0)
        {
            ...
            if(g_debugDump >= 2)
            {
                ...
                eisAlgoLogInfo.Eis_Log_Buf_Addr = mEisDbgBuf.virtAddr;
    ...
    else if(aEisPass == EIS_PASS_2)
    {
        ...
        else
        {
            ...
            if(mIsEisPlusConfig == 0)
            {
                ...
                eisPlusWorkBufData.extMemStartAddr = mEisPlusWorkBuf.virtAddr;
...
{    
    ...
    if(aEisPass == EIS_PASS_1)
    {
        ...
        else
        {
            ...
            {
                mEisAlgoProcData.AcceInfo[i] = gAccInfo[i];
                mEisAlgoProcData.GyroInfo[i] = gGyroInfo[i];            
            }

            if(g_debugDump >= 1)
            {
                EIS_LOG("Acc(%f,%f,%f)",mEisAlgoProcData.AcceInfo[0],mEisAlgoProcData.AcceInfo[1],mEisAlgoProcData.AcceInfo[2]);
                EIS_LOG("Gyro(%f,%f,%f)",mEisAlgoProcData.GyroInfo[0],mEisAlgoProcData.GyroInfo[1],mEisAlgoProcData.GyroInfo[2]);
            }
