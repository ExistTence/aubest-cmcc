...
void FlashMgrM::hw_setExpPara(FlashAlgExpPara* expPara, int sensorType, FLASH_PROJECT_PARA* pPrjPara)
{
	...
	if(isFlash)
	{
		if(mFlash1Mode!=LIB3A_FLASH_MODE_FORCE_TORCH)	
			turnOnPf();
	}
	else
	{
...
int FlashMgrM::setFlashMode(int mode)
{
	...
	logI("------m_isAfState=[%d]---g_previewMode=[%d]----\n",m_isAfState,g_previewMode);
	 //songwei add for flash mode error. 	2080
	AeMgr::getInstance(m_sensorDev).setStrobeMode(MFALSE);	
