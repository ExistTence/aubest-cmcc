...
/*sonwei add 2014 7-14 for flash node begin */
    err = ioctl(a_fdMCU,DW9718AFIOC_T_MOVETO,(unsigned long)a_i4FocusPos);
    if (err < 0) {
        DRV_ERR("[moveMCU] ioctl - DW9718AFIOC_T_MOVETO, error %s",  strerror(errno));
        return err; 
    }
/*sonwei add 2014 7-14 for flash node end */ 
    return MCUDrv::MCU_NO_ERROR;
}
...
LensDrv::getMCUInfo(mcuMotorInfo *a_pMotorInfo, unsigned int a_u4CurrSensorDev )
{
	...
	{	
		...
		return MCUDrv::MCU_INVALID_DRIVER;
/*sonwei add 2014 7-14 for flash node begin */
    stDW9718AF_MotorInfo motorInfo; 
    memset(&motorInfo, 0, sizeof(stDW9718AF_MotorInfo)); 
/*sonwei add 2014 7-14 for flash node end */
    if (a_fdMCU < 0) {
...
/*sonwei add 2014 7-14 for flash node begin */
    err = ioctl(a_fdMCU,DW9718AFIOC_G_MOTORINFO, &motorInfo);
    if (err < 0) {
        DRV_ERR("[getMCUInfo] ioctl - DW9718AFIOC_G_MOTORINFO, error %s",  strerror(errno));
        return err;         
    }
/*sonwei add 2014 7-14 for flash node end */ 
...
/*sonwei add 2014 7-14 for flash node begin */
    err = ioctl(a_fdMCU,DW9718AFIOC_T_SETINFPOS,(unsigned long)a_i4FocusPos);
    if (err  < 0) {
        DRV_ERR("[setMCUInfPos] ioctl - DW9718AFIOC_T_SETINFPOS, error %s",  strerror(errno));
        return err;             
    }
/*sonwei add 2014 7-14 for flash node end */
...
/*sonwei add 2014 7-14 for flash node begin */
    err = ioctl(a_fdMCU,DW9718AFIOC_T_SETMACROPOS,(unsigned long)a_i4FocusPos);
    if (err < 0) {
        DRV_ERR("[setMCUMacroPos] ioctl - DW9718AFIOC_T_SETMACROPOS, error %s",  strerror(errno));
        return err;          
    }
/*sonwei add 2014 7-14 for flash node end */
    return MCUDrv::MCU_NO_ERROR;
...
/*sonwei add 2014 7-14 for flash node begin */    
	/*
    err = ioctl(a_fdMCU,DW9718AFIOC_T_SETPARA,ois_param);
    if (err < 0) {
        DRV_ERR("[setMCUMacroPos] ioctl - DW9718AFIOC_T_SETPARA, error %s",  strerror(errno));
     return err;          
    }
	*/
/*sonwei add 2014 7-14 for flash node end */	
