...
SingleShot(
    ...
    , mbIVHdr(MFALSE)
    , mIVhdrProfile(EIspProfile_IHDR_Preview)
{
}
...
startOne(SensorParam const & rSensorParam)
{
    ...
    //query flash on/off
    {
        ...
        if( !isYuv                                          &&  // not yuv sensor
            ...
            mbIVHdr                                             // if ivhdr for cct
            )
        {
            mbPureRaw = MTRUE;
            MY_LOGD("flash %d, rawtype %d, ivhdr %d: use pureraw",
                    bFlashOn, mSensorParam.u4RawType, mbIVHdr);
        }
...
sendCommand(MINT32 cmd, MINT32 arg1, MINT32 arg2, MINT32 arg3)
{
    ...
    switch( cmd )
    {
        ...
        case ECamShot_CMD_SET_NRTYPE:
            ...
            break;
        case ECamShot_CMD_SET_IVHDR_PROFILE:
            mbIVHdr = MTRUE; //enable ivhdr mode
            mIVhdrProfile = (EIspProfile_T)arg1;
            MY_LOGD("set ivhdr profile 0x%x", arg1);
            break;
        default:
            break;
    }
...
//    if( mShotParam.u4PictureTransform & eTransform_ROT_90 ) // 90, 270
      //songwei add for front camera capture mirror mode.
     if( mShotParam.u4PictureTransform == eTransform_ROT_90 ||
      ...
      //songwei add for front camera capture mirror mode end.
    {
