...
//zhouwl,2014-08-28,temp for test
...
status_t AudioALSACodecDeviceOutSpeakerNXP::open(const uint32_t SampleRate)
{
    ...
    if (mClientCount == 0)
    {
        ...
        else if (SampleRate == 16000)
        {
            ...
            ALOGD("sample rate 16k Audio_i2s0_SideGen_Switch(On16000EchoRef)");

            if (mixer_ctl_set_enum_by_string(mixer_get_ctl_by_name(mMixer, "Audio_i2s0_SideGen_Switch"), "On16000EchoRef"))
            {
                ALOGE("Error: Audio_i2s0_SideGen_Switch invalid value");
            }
#else
